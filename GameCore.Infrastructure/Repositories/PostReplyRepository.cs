...
namespace GameCore.Infrastructure.Repositories
{
    public class PostReplyRepository : Repository<PostReply>, IPostReplyRepository
    {
        ...
        public PostReplyRepository(GameCoreDbContext context, ILogger<PostReplyRepository> logger) : base(context)
        {
            _logger = logger;
        }
        ...
        public async Task<IEnumerable<PostReply>> GetByPostIdAsync(int postId, CancellationToken cancellationToken = default)
        {
            return await GetByPostIdAsync(postId, 1, 50);
        }

        public async Task<IEnumerable<PostReply>> GetByUserIdAsync(int userId, CancellationToken cancellationToken = default)
        {
            return await GetByAuthorIdAsync(userId, 1, 50);
        }

        public async Task<IEnumerable<PostReply>> GetByStatusAsync(string status, CancellationToken cancellationToken = default)
        {
            if (Enum.TryParse<PostStatus>(status, true, out var parsed))
            {
                return await GetByStatusAsync(parsed, 1, 50);
            }
            return new List<PostReply>();
        }

        public async Task<int> GetReplyCountAsync(int postId, CancellationToken cancellationToken = default)
        {
            return await GetReplyCountByPostAsync(postId);
        }

        public async Task<bool> UpdateStatusAsync(int replyId, string status, CancellationToken cancellationToken = default)
        {
            var reply = await GetByIdAsync(replyId);
            if (reply == null) return false;
            if (!Enum.TryParse<PostStatus>(status, true, out var parsed)) return false;
            reply.Status = parsed;
            _context.PostReplies.Update(reply);
            return true;
        }

        public async Task<IEnumerable<PostReply>> GetReplyTreeAsync(int postId, CancellationToken cancellationToken = default)
        {
            return await GetRepliesByThreadAsync(postId);
        }
    }
}